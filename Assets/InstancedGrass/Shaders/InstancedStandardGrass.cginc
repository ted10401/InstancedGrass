...
void rotateRandom(inout VertexInput v, float4 position)
{
	float rotation = position.x + position.y * 10 + position.z * 100 + position.w * 1000;
	rotate2D(v.vertex.xz, rotation);
}

void setupObjectToWorld(float4 position)
{
	float scale = position.w * _GrassSize;
...
void updateStamp(inout VertexInput v, float4 position)
{
...
void updateWind(inout VertexInput v, float4 position)
{
...
VertexOutputDeferred vertDeferred (VertexInput v, uint instanceID : SV_InstanceID)
{
    UNITY_SETUP_INSTANCE_ID(v);
